...
- **Students/Explorers**: Name → Age range tile → Pick a codename from a visual grid → Done
...
- **Codenames**: Pre-loaded grid — student picks the one they like
...
~60 codenames organized by vibe. Each has a name, an emoji icon, and a color. Mix of African-origin and universal names.

### Sample Library

| Codename | Origin | Icon | Color | Meaning |
|----------|--------|------|-------|---------|
...
Full library: 60 names stored in `htdocs/config/codenames.json`

### Codename Rules
- Once picked, a codename is taken — no two users can have the same one
- Codename displayed prominently (it's their identity in the app)
- Real name stored in DB but codename is what others see
...
CREATE TABLE IF NOT EXISTS users (
    ...
    codename       VARCHAR(50) DEFAULT NULL COMMENT 'Chosen codename (students only)',
    codename_icon  VARCHAR(10) DEFAULT NULL COMMENT 'Emoji icon for codename',
    codename_color CHAR(7) DEFAULT NULL COMMENT 'Accent color for codename',
...
$_SESSION['codename']     = string;   // Codename (students) or null (teachers)
...
| `htdocs/register.php` | Student signup: name → age range → codename picker |
...
| `htdocs/config/codenames.json` | 60 codenames with icons, colors, meanings |
| `htdocs/css/login.css` | Login/register page styles |
| `htdocs/js/login.js` | Client-side name filter, codename picker, step wizard |
| `db/migrations/0004_update_users_table.sql` | Schema updates for codenames + teacher auth |
...
- Grid of existing user cards showing: **codename** (large), emoji icon, real name (small)
...
- Clean, friendly, colorful — each card uses the codename's accent color
...
- **Step 3**: "Pick your codename!" — grid of available codenames, each showing icon + name + color. Taken names greyed out. Tap to select, confirm.
- **Welcome screen**: "Welcome, [Name]! You are now [CODENAME] [icon]" with a celebration animation
...
function getUserDisplay(): string {
    // Returns "CODENAME 🦁" for students, "Name" for teachers
}
```

### Switch user
- Always visible in nav: "[CODENAME icon] · Switch"
...
| Screen | User cards | Codename picker | Age tiles |
|--------|-----------|-----------------|-----------|
...
| 2 | codenames.json: 60 names with icons/colors | 0.25d | — |
...
| 9 | login.js: wizard steps, name filter, codename picker | 0.5d | 5, 6 |
...
| Case | Handling |
|------|---------|
| Duplicate real name "Kofi" | Both can exist — codenames make them unique |
| All 60 codenames taken | Generate "[Name] the [Adjective]" combos (Brave, Swift, Wise, etc.) |
...
| T2 | Student signup: name → age → codename | Creates user, sets session, redirects to / |
| T3 | Return to /login.php | Shows new user card with codename |
| T4 | Tap user card → confirm → login | Session set, redirect to / |
| T5 | Pick codename already taken | Codename appears greyed out / unavailable |
...
| T12 | Mobile (320px) codename grid | 2 columns, all tappable |
| T13 | Search filter on returning user grid | Filters by codename and real name |
...
2. Avatar photos — skip for v1, use codename icon + color as visual identity
