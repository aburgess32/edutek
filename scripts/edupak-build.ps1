...
#
# Output:
#   edupak-update-YYYYMMDD-HHMMSS.zip in the dist\ folder
#   (or in -OutputDir if specified)
# ============================================================
...
if (-not $SkipPull) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "Git pull failed — building from current state"
    }
} else {
...
foreach ($dir in $appDirs) {
    ...
    if (Test-Path $src) {
        Copy-Item $src -Destination (Join-Path $htdocsStaging $dir) -Recurse -Force
    }
}
...
if (Test-Path $htaccess) {
    Copy-Item $htaccess -Destination $htdocsStaging -Force
}

# Database migrations
$dbSource = Join-Path $projectRoot "db"
if (Test-Path $dbSource) {
    Copy-Item $dbSource -Destination (Join-Path $stagingDir "db") -Recurse -Force
}

# Scripts (for on-device use)
...
foreach ($sf in $scriptFiles) {
    ...
    if (Test-Path $src) {
        Copy-Item $src -Destination $scriptsStaging -Force
    }
}

# .env.example (for reference, never overwrites .env)
$envExample = Join-Path $projectRoot ".env.example"
if (Test-Path $envExample) {
    Copy-Item $envExample -Destination $stagingDir -Force
}
...
============
Archive    : $archiveName
Built at   : $(Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ")
Git commit : $commitHash
Git branch : $commitBranch
Builder    : $env:USERNAME@$env:COMPUTERNAME
"@
$buildInfo | Out-File -FilePath (Join-Path $stagingDir "BUILD_INFO.txt") -Encoding UTF8

# -- Safety check: refuse to package large directories --
